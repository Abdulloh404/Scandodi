...
<head>
    ...
    <link href="{{ asset('front/css/signIn') }}" rel="stylesheet">
</head>
...
<body class="{{ $siteDirection }}" direction="{{ $siteDirection }}">
    ...
    <div id="main-wrapper">
        <div class="nav-header">
            @if (isset(auth()->user()->type) && auth()->user()->type == 'customer' && $modules)
                <a href="{{ route('multirestaurant::index') }}" class="brand-logo">
                    <img class="logo-abbr"
                        src="{{ asset('uploads/' . json_decode(get_settings('site_setting'))->logo) }}" alt="">
                            ...
                            <ul class="navbar-nav header-right">
                                <li class="nav-item dropdown notification_dropdown">
                                    @if (auth()->user()->type == 'customer')
                                        @if (request()->get('table') && request()->get('id') && !$modules)
                                            <form action="{{ route('call.waiter.store') }}" method="post"
                                                class="{{ request()->segment('1') == 'orders' ? 'd-none' : '' }}{{ request()->segment('1') == 'settings' ? 'd-none' : '' }}">
                                                @csrf
                                                ...
                                                <button type="submit"
                                                    class="btn btn-sm badge-success">{{ trans('Call Waiter') }}</button>
                                            </form>
                                        @else
                                            @if ($modules)
                                                <a class="nav-link bell bell-link primary"
                                                    href="{{ route('multirestaurant::addToCart') }}">
                                            ...
                                            @else
                                                <button type="btn"
                                                    class="btn btn-sm badge-success call_waiter_btn {{ request()->segment('1') == 'orders' ? 'd-none' : '' }}{{ request()->segment('1') == 'settings' ? 'd-none' : '' }}">{{ trans('Call Waiter') }}</button>
                                            @endif
                                        @endif
                                    @endif
                                ...
                                @if (auth()->user()->type == 'restaurant_owner' ||
                                    ...
                                    <li class="nav-item dropdown notification_dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                                        ...
                                                        <li class="notification-{{ $notification->id }}">
                                                            <div class="timeline-panel">
                                                                ...
                                                                <div class="media-body">
                                                                    ...
                                                                    @elseif($notification->type == 'call_waiter')
                                                                        <h6 class="mb-1"><a
                                                                                href="{{ route('call.waiter') }}">{{ $notification->message }}</a>
                                                                        </h6>
                                                                    @else
                                                                        <h6 class="mb-1">
                                                                    ...
                                                                    @endif
                                                                    <small class="notification_created_at"
                                            ...
                                            <a class="all-notification d-none"
                                                href="#">{{ trans('layout.all_notification') }} <i
                                ...
                                <li class="nav-item dropdown header-profile">
                                    <a class="nav-link" href="#" role="button" data-toggle="dropdown">
                                        <div class="header-info">
                                            <span>{{ trans('layout.hello') }}
                                                <strong>{{ short_word(auth()->user()->name, 1, '') }}</strong></span>
                                        </div>
                                        <img src="{{ asset('uploads/' . auth()->user()->picture) }}" width="20"
                                            alt="" />
                                    </a>
                                ...
                                <ul class="navbar-nav header-right">
                                    <li class="nav-item dropdown notification_dropdown">
                                        @if (!auth()->user() || auth()->user()->type == 'customer')
                                            @if (request()->get('table') && request()->get('id'))
                                                <form action="{{ route('call.waiter.store') }}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="restaurant"
                                                        value="{{ request()->get('id') }}">
                                            ...
                                            @else
                                                <button type="btn"
                                                    class="btn btn-sm badge-success call_waiter_btn">{{ trans('Call Waiter') }}</button>
                                            @endif
                                        @endif
                                    </li>
                                    <li class="nav-item dropdown notification_dropdown">
                                        <a class="nav-link bell bell-link primary"
                                            href="{{ route('registration', ['type' => 'customer', 'restaurant' => request()->get('id'), 'slug' => request()->segment(2)]) }}">
        ...
        <div class="footer">
            <div class="copyright text-center">
                @if (isset($customFooter))
                    {{ $customFooter }}
                @else
                    <p>{{ trans('layout.copyright_footer') }} <a
                            href="#">{{ json_decode(get_settings('site_setting'))->name }}</a>
                        {{ date('Y') }}</p>
                @endif
            </div>
        </div>
                    ...
                    <div class="modal-body">
                        <div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input name="time" required type="number" class="form-control"
                                    placeholder="Ex: 20">
                            </div>
                            <div class="form-group col-md-6">
    ...
    <script>
        $(document).on('click', '.call_waiter_btn', function(e) {
    ...
    @if (auth()->check() &&
        isset($site_setting) &&
        isset($site_setting->live_notification) &&
        $site_setting->live_notification == 'enable')
