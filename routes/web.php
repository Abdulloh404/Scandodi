...
Route::get('/blog-detail', [\App\Http\Controllers\BlogDetailController::class, 'showblogdetail']);
Route::get('/policy', [\App\Http\Controllers\PolicyController::class, 'showpolicy']);
    ...
    try {
        ...
        if ($emailTemplate) {
            ...
            Mail::send('sendMail', ['htmlData' => $regTemp], function ($message) use ($user, $emailTemplate) {
                $message->to($user->email)->subject($emailTemplate->subject);
            });
        }
    } catch (\Exception $ex) {
    }
