<ul class="metismenu sidebar-height" id="menu">
    @if (auth()->user()->type == 'customer')
        @php
            $customer = auth()->user();
            $menu = \App\Models\Menu::where('user_id', $customer->id)
    ...
    @if (auth()->user()->type == 'restaurant_owner')
        <li class="{{ isSidebarActive('live.order') }} active-no-child"><a class="ai-icon"
                href="{{ route('live.order') }}" aria-expanded="false">
                <i class="flaticon-381-notepad "></i>
                <span class="nav-text">{{ trans('layout.live_order') }} <div class="pulse-css live-order-blink"></div>
                </span>
            </a>
    ...
    @if (auth()->user()->type == 'restaurant_owner')
        <li class="{{ isSidebarActive('call.waiter') }} active-no-child"><a class="ai-icon"
                href="{{ route('call.waiter') }}" aria-expanded="false">
    ...
    @can('table_booking_manage')
        @if ($modules)
            <li class="{{ isSidebarActive('multirestaurant::tables.booking') }} active-no-child"><a class="ai-icon"
                    href="{{ route('multirestaurant::tables.booking') }}" aria-expanded="false">
                    <i class="flaticon-381-television"></i>
                    <span class="nav-text">{{ trans('multirestaurant::layout.table_booking') }}<small
                            class="addon">({{ trans('multirestaurant::layout.addon') }})</small></span>
                </a>
            </li>
        @endif
    @endcan
    @can('billing')
        <li class="{{ isSidebarActive('billings') }} active-no-child"><a class="ai-icon" href="{{ route('billings') }}"
                aria-expanded="false">
    ...
    @can('restaurant_manage')
        <li class="{{ isSidebarActive('restaurant*') }} active-no-child"><a class="ai-icon"
                href="{{ route('restaurant.index') }}" aria-expanded="false">
                <i class="flaticon-381-television"></i>
    ...
    @can('user_plan_change')
        <li class="{{ isSidebarActive('user.plan') }} active-no-child"><a class="ai-icon"
                href="{{ route('user.plan') }}" aria-expanded="false">
                <i class="flaticon-381-network "></i>
    ...
    @can('city_manage')
        @if ($modules)
            <li class="{{ isSidebarActive('multirestaurant::cities') }} active-no-child"><a class="ai-icon"
                    href="{{ route('multirestaurant::cities') }}" aria-expanded="false">
                    <i class="ti-map"></i>
                    <span class="nav-text">{{ trans('multirestaurant::layout.cities') }}@if (env('APP_DEMO'))
                            <small class="addon">({{ trans('multirestaurant::layout.addon') }})</small>
                        @endif
                    </span>
                </a>
            </li>
    ...
    @can('template_manage')
        <li><a class="has-arrow ai-icon" href="javascript:void(0)" aria-expanded="false">
            ...
            <ul aria-expanded="false">
                @if ($modules)
                    <li>
                        <a href="{{ route('multirestaurant::multirestaurant.template') }}">{{ trans('multirestaurant::layout.multi_restaurant') }}
                            @if (env('APP_DEMO'))
                                <small class="addon">({{ trans('multirestaurant::layout.addon') }})</small>
                            @endif
                        </a>
    ...
    @if (auth()->user()->type != 'user')
        <li class="{{ isSidebarActive('settings*') }} active-no-child"><a href="{{ route('settings') }}"
    ...
    @endcan
    @can('addon_manage')
        <li class="{{ isSidebarActive('addon.index') }} active-no-child"><a class="ai-icon"
                href="{{ route('addon.index') }}" aria-expanded="false">
                <i class="ti-plug"></i>
