                                ...
                                @if(auth()->user()->type=='customer')
                                    @if(request()->get('table') && request()->get('id') && !$modules)
                                        <form action="{{route('call.waiter.store')}}" method="post"
                                            ...
                                            @csrf
                                            <input type="hidden" name="restaurant" value="{{request()->get('id')}}">
                                    ...
                                    @else
                                        @if($modules)
                                            <a class="nav-link bell bell-link primary" href="{{route('multirestaurant::addToCart')}}">
                                        ...
                                        @else
                                            <button type="btn" class="btn btn-sm badge-success call_waiter_btn {{request()->segment('1')=='orders'?'d-none':''}}{{request()->segment('1') =='settings'?'d-none':''}}">{{trans('Call Waiter')}}</button>
                                        @endif
                            @endif
                        @endif
                            ...
                            @if(auth()->user()->type=='restaurant_owner' || auth()->user()->type=='admin' || auth()->user()->type=='user')
                                    <li class="nav-item dropdown notification_dropdown">
                                                ...
                                                <li class="notification-{{$notification->id}}">
                                                    <div class="timeline-panel">
                                                        ...
                                                        <div class="media-body">
                                                            ...
                                                            @else
                                                                <h6 class="mb-1"><span>{{$notification->message}}</span>
                                                                </h6>
                                                            @endif
                                                            <small
                                                                class="notification_created_at" data-date="{{$notification->created_at}}">{{$notification->created_at->diffForHumans()}}</small>
                                                        </div>
                                                    </div>
                                                </li>
                                            @endforeach
                                    ...
                                    @if(!auth()->user() || auth()->user()->type=='customer')
                                        @if(request()->get('table') && request()->get('id'))
                                            <form action="{{route('call.waiter.store')}}" method="post">
                                                @csrf
                                                <input type="hidden" name="restaurant" value="{{request()->get('id')}}">
                                                <input type="hidden" name="table_id" value="{{request()->get('table')}}">
                                                <button type="submit"
                                                        class="btn btn-sm badge-success">{{trans('Call Waiter')}}</button>
                                            </form>
                                        @else
    ...
    <div class="footer">
        <div class="copyright text-center">
            @if(isset($customFooter))
                {{$customFooter}}
            @else
                <p>{{trans('layout.copyright_footer')}} <a href="#" >{{json_decode(get_settings('site_setting'))->name}}</a> {{date('Y')}}</p>
            @endif
        </div>
    </div>
